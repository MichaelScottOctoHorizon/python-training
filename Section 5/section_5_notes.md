...
### For loops
For loops are mostly ideal when you: Know how many times you want to loop Used for iterating over lists Running a fixed number of items

1. Example 1 - for number in range(20):
2. Example 2 - for item in [“Steak”, “Ribs”, “Samon”, “Wings”]:
3. Example 3 - for day in [“Friday”, “Saturday”, “Sunday”]
...
### Break
this is used used to completely break out of a loop without needing the loop condition to be met.
...
### Continue
This is used to skip any code within the loop from executing that occurs after it. After continue is activated, it will automatically skip code from executing in that current iteration of the loop but if the loop still has more iterations left it will still continue to run from the next iteration.
...
### Pass
this is used as a placeholder  for bypassing an error being flagged within an empty loop, function, or if statement ect. It is useful because you may not always know what you want to put within a body of code right away but you still want the code to run.
...
## 4. Common loop mistakes

Some common loop mistakes are as follows:

1. **Off-By-One Error** - This occurs when your range is either 1 too high or 1 too low.  
...
2. **Infinite loops** - This can occur if a coder forgets to implement a loop control variable or an exit condition to exit the loop on meeting the condition.  
...
3. **Forgetting to update loop variable** - In a while loop you may have initialised a loop variable called ‘count’ to 0, but within the loop you forgot to either increment or decrement it by an amount like  
   - **Example 1** - If the loop is ‘while count < 5:’ the loop will endlessly loop due to count being 0 and never getting to 5. To fix this always remember to increment and decrement the loop variables and test it many times.  
   - **Example 2** - If you use break in a loop that is meant to sum up a list of integers it may skip summing up the remaining numbers in the list giving you an incorrect sum.  
   - **Example 3** - if you use continue in the wrong place like right before you increment a count variable in the while loop the loop will never end as the variable never increments.  
   - **Example 4** - using return in the wrong place or too early may cause important logic from running. Like if you put a return statement before the database logic in a function to update the user in the Dateabse the user will not be updated.
...
## 6. Tips for infinite loops are as follows:

1. Always ensure you have a loop control variable
2. Always ensure there is some criteria to exit a loop.
3. Always ensure you place your breaks, continue, and returns in the correct place
4. Always test code many times and try to break it to detect potential bugs.
5. Take your time to ensure your logic is tested on many different types of data.

---

## 7. Section 4 Lessons learned:
